...
export async function ensureDb(){if(!initialized)initialized=(async()=>{const binding=d1();const addCols=["ALTER TABLE orders ADD COLUMN utm_campaign text","ALTER TABLE orders ADD COLUMN utm_source text","ALTER TABLE orders ADD COLUMN utm_medium text","ALTER TABLE orders ADD COLUMN utm_content text","ALTER TABLE orders ADD COLUMN utm_term text"];if(binding){for(const statement of statements)await binding.prepare(statement).run();for(const add of addCols){try{await binding.prepare(add).run()}catch{}}}else{await getSql().unsafe(statements.map(s=>s+';').join('\n'));for(const add of addCols){try{await getSql().unsafe(add)}catch{}}}})().catch(error=>{initialized=undefined;throw error});return initialized}
