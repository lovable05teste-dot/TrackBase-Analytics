...
function initialMode():"login"|"register"{try{const m=(new URLSearchParams(window.location.search).get("modo")||"").toLowerCase();return m==="register"||m==="cadastro"||m==="criar"||m==="criar-conta"?"register":"login"}catch{return "login"}}
function initialOauthError(){try{const e=new URLSearchParams(window.location.search).get("erro");return e?oauthErrors[e]||"Não foi possível entrar com o Google.":""}catch{return ""}}

export default function Login(){
 const[mode,setMode]=useState<"login"|"register">(initialMode);
 const[email,setEmail]=useState("");const[password,setPassword]=useState("");const[show,setShow]=useState(false);
  const[name,setName]=useState("");const[cpf,setCpf]=useState("");const[confirmPassword,setConfirmPassword]=useState("");const[need2fa,setNeed2fa]=useState(false);const[code,setCode]=useState("");const[needVerification,setNeedVerification]=useState(false);const[setup2fa,setSetup2fa]=useState(false);const[setupSecret,setSetupSecret]=useState("");
 const[remember,setRemember]=useState(true);const[terms,setTerms]=useState(false);const[error,setError]=useState(initialOauthError);const[loading,setLoading]=useState(false);
   async function submit(e:FormEvent){e.preventDefault();setLoading(true);setError("");try{if(setup2fa){const r=await fetch("/api/auth/2fa/confirm",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({code})});const b=await r.json();if(!r.ok)throw new Error(b.error||"Código inválido.");const back=new URLSearchParams(window.location.search).get("return_to");location.href=back&&back.startsWith("/")&&!back.startsWith("//")?back:"/";return}if(need2fa){const r=await fetch("/api/auth/2fa/verify",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({code})});const b=await r.json();if(!r.ok)throw new Error(b.error||"Código inválido.");const back=new URLSearchParams(window.location.search).get("return_to");location.href=back&&back.startsWith("/")&&!back.startsWith("//")?back:"/";return}if(mode==="register"&&!terms)throw new Error("Aceite os Termos de Uso para criar sua conta.");if(mode==="register"&&!validateCpf(cpf))throw new Error("Confira o CPF digitado.");if(mode==="register"&&password!==confirmPassword)throw new Error("As senhas não coincidem.");const url=mode==="register"?"/api/auth/register":"/api/auth/login";const payload=mode==="register"?{name:name.trim(),email:email.trim(),cpf:stripCpf(cpf),password,confirmPassword}:{email:email.trim(),password};const r=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});const b=await r.json();if(!r.ok)throw new Error(b.error||"Não foi possível entrar.");if(b.require2fa){setNeed2fa(true);return}if(b.needVerification){setNeedVerification(true);return}if(b.require2faSetup){const s=await fetch("/api/auth/2fa/setup",{method:"POST"});const sb=await s.json();if(!s.ok)throw new Error(sb.error||"Não foi possível iniciar o 2FA.");setSetupSecret(sb.secret||"");setSetup2fa(true);return}if(mode==="register"){location.href="/verificar-codigo?email="+encodeURIComponent(email.trim());return}if(!remember)sessionStorage.setItem("tb_noremember","1");const back=new URLSearchParams(window.location.search).get("return_to");location.href=back&&back.startsWith("/")&&!back.startsWith("//")?back:"/";}catch(e){setError(e instanceof Error?e.message:"Não foi possível entrar.");}finally{setLoading(false)}}
 const submitLabel=mode==="register"?"Criar conta grátis":"Entrar no painel";
   const registerValid=name.trim().length>=3&&email.includes("@")&&validateCpf(cpf)&&password.length>=8&&/[A-Za-z]/.test(password)&&/[0-9]/.test(password)&&/[A-Z]/.test(password)&&confirmPassword.length>0&&terms;
 ...
 <section className="relative hidden overflow-hidden lg:flex lg:flex-col lg:justify-between lg:p-12">
  ...
  <div className="relative mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-white/10 pt-6 text-sm">
   ...
   <div><b className="text-lg">Planos</b><small className="block text-xs text-slate-500">a partir de R$ 39,90/mês</small></div>
  ...
  <div className="rounded-2xl border border-white/10 bg-[#101521] p-6 shadow-[0_16px_60px_#0006] sm:p-8">
   ...
   <form onSubmit={submit} className="mt-5 space-y-4">
    ...
    <div><label htmlFor="tb-password" className="mb-1.5 flex items-center gap-1.5 text-sm text-slate-300"><Lock className="size-3.5 text-slate-500"/> Senha</label>
     <div className="relative">      <Input id="tb-password" type={show?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder={mode==="register"?"Mínimo 8, maiúscula e número":"••••••••"} required minLength={mode==="register"?8:1} autoComplete={mode==="register"?"new-password":"current-password"} className="h-12 pr-11"/>
    ...
    {mode==="register"&&<div className="flex items-start gap-2.5 rounded-xl border border-white/10 bg-white/[.02] p-3"><input id="tb-terms" type="checkbox" checked={terms} onChange={e=>setTerms(e.target.checked)} className="mt-0.5 size-4 shrink-0 accent-red-600"/><span className="text-xs leading-relaxed text-slate-400">Li e aceito os <a href="/termos" className="font-medium text-slate-200 underline underline-offset-2 hover:text-white">Termos de Uso</a> e a <a href="/privacidade" className="font-medium text-slate-200 underline underline-offset-2 hover:text-white">Política de Privacidade</a>, incluindo a cobrança do plano e R$ 0,10 por venda excedente.</span></div>}
    <div className="flex items-center justify-between text-sm"><label className="flex cursor-pointer items-center gap-2 text-slate-400"><input type="checkbox" checked={remember} onChange={e=>setRemember(e.target.checked)} className="size-4 accent-red-600"/>Manter conectado</label><a href="/docs/faq" className="text-slate-400 hover:text-slate-200">Precisa de ajuda?</a></div>
         {mode==="login"&&!need2fa&&<div className="flex justify-end text-sm"><a href="/recuperar" className="text-slate-400 hover:text-slate-200">Esqueci a senha</a></div>}
     {need2fa&&<div><label htmlFor="tb-code" className="mb-1.5 flex items-center gap-1.5 text-sm text-slate-300"><Lock className="size-3.5 text-slate-500"/> Código do autenticador</label>
      <Input id="tb-code" type="text" inputMode="numeric" value={code} onChange={e=>setCode(e.target.value)} placeholder="000000" required autoComplete="one-time-code" maxLength={8} className="h-12"/></div>}
     {needVerification&&<div className="rounded-xl border border-amber-400/25 bg-amber-400/[.07] px-3.5 py-2.5 text-xs leading-relaxed text-amber-200">Confirme seu e-mail para entrar.<a href={`/verificar-codigo?email=${encodeURIComponent(email)}`} className="ml-2 font-medium underline underline-offset-2 hover:text-amber-100">Confirmar agora</a></div>}
     {setup2fa&&<div className="rounded-xl border border-white/10 bg-white/[.02] p-3 text-xs leading-relaxed text-slate-300">O 2FA é obrigatório nesta conta. Abra seu app autenticador, adicione esta chave:<code className="mt-1 block select-all break-all rounded-lg bg-black/40 p-2 font-mono text-[13px] text-emerald-200">{setupSecret}</code><span className="mt-2 block">Depois digite o código de 6 dígitos abaixo e confirme.</span></div>}
     {setup2fa&&<div><label htmlFor="tb-code" className="mb-1.5 flex items-center gap-1.5 text-sm text-slate-300"><Lock className="size-3.5 text-slate-500"/> Código do autenticador</label>
      <Input id="tb-code" type="text" inputMode="numeric" value={code} onChange={e=>setCode(e.target.value)} placeholder="000000" required autoComplete="one-time-code" maxLength={8} className="h-12"/></div>}
     {error&&<p role="alert" className="rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-2.5 text-sm text-red-200">{error}</p>}
    <Button type="submit" disabled={loading||(mode==="register"?!registerValid:((need2fa||setup2fa)?!code:needVerification?true:(!email||!password)))} className="group h-12 w-full bg-[#ff0030] text-[15px] font-semibold text-white hover:bg-[#d60029] hover:shadow-[0_12px_36px_-8px_#ff0030aa] disabled:opacity-60">{loading?<span className="inline-flex items-center gap-2"><Loader2 className="size-4 animate-spin"/> Entrando...</span>:<span className="inline-flex items-center gap-2.5">{submitLabel}<span className="grid size-7 place-items-center rounded-full bg-white/20 transition-all duration-300 group-hover:scale-125 group-hover:bg-white group-hover:text-[#ff0030]"><ArrowRight className="size-4 transition-transform duration-300 group-hover:translate-x-0.5" strokeWidth={2.75}/></span></span>}</Button>
