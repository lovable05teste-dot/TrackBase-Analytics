...
export default async function Page() {
  ...
  // previous 7d window: manual calc via totals diff is complex; approximate with last30-last7 is skipped — use simple projection
  ...
  // Projeção ingênua: média diária dos últimos 7d * 7
  const projPurch = Math.round((cPurch / 7) * 7);
  const projRev = projPurch * avgTicket;
  const trend30 = (last30.byName.get("Purchase") ?? 0) / 30;
  const trend7 = cPurch / 7;
  const trend = trend7 > trend30 ? "alta" : trend7 < trend30 ? "queda" : "estável";
