...
export function WebhooksClient() {
  ...
  async function test() {
    ...
    try {
      ...
      if (!["http:", "https:"].includes(parsed.protocol) || parsed.username || parsed.password) throw new Error("Use uma URL HTTP ou HTTPS sem credenciais no endereço.");
      const r = await fetch(target, { method: "POST", headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: payload, credentials: "omit", referrerPolicy: "no-referrer", signal: AbortSignal.timeout(15000) });
  ...
  return (
    <div className="grid gap-4">
      <Card className="metric-card">
        ...
        <CardContent className="grid gap-3"><p className="text-sm text-amber-500">Envio real: use a credencial de um projeto de testes. Um payload aprovado pode registrar uma venda e enviar Purchase.</p>
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label className="text-sm">Token (tb_live_...)<input type="password" autoComplete="off" value={token} onChange={(e) => setToken(e.target.value)} placeholder="tb_live_..." className="mt-1 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm" /></label>
          </div>
