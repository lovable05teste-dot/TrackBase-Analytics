...
export function MetaAccountsClient(){
 ...
 useEffect(()=>{load();},[]);
 const select=async(id:string)=>{setSaving(id);setError("");try{const r=await fetch("/api/meta/accounts",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({accountId:id})});const b=await readJson(r);if(!r.ok)throw new Error(b.error||"Falha ao selecionar.");await load();}catch(e){setError(e instanceof Error?e.message:"Falha ao selecionar.");}finally{setSaving("");}};
