...
export function MetaAccountsClient(){
 ...
 const load=async()=>{setError("");const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),8000);try{const r=await fetch("/api/meta/accounts",{signal:controller.signal});const b=await r.json();if(!r.ok)throw new Error(b.error);setData(b);}catch(e){setError(e instanceof DOMException&&e.name==="AbortError"?"A lista de contas demorou para responder. Você ainda pode iniciar uma nova conexão.":e instanceof Error?e.message:"Não foi possível carregar.");}finally{clearTimeout(timer)}};
 useEffect(()=>{load();},[]);
 ...
 return <div className="space-y-5">
   ...
   {accounts.length===0?<div className="rounded-xl border border-white/10 p-6 text-slate-400">Abra o botão acima neste navegador ou copie o link para um perfil do AdsPower. Depois da autorização, as contas liberadas pela Meta aparecerão aqui.</div>:<div className="grid gap-3">{accounts.map(a=><div key={a.id} className={`flex flex-col gap-4 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between ${a.selected?"border-violet-400/60 bg-violet-500/10":"border-white/10"}`}><div><div className="flex items-center gap-2"><strong>{a.name}</strong>{a.selected&&<span className="inline-flex items-center gap-1 text-sm text-emerald-300"><CheckCircle2 className="size-4"/>Conectada</span>}</div><p className="mt-1 text-sm text-slate-400">act_{a.adAccountId} · {a.currency||"Moeda não informada"} · {a.timezoneName||"Fuso não informado"}</p></div><Button variant={a.selected?"secondary":"default"} disabled={a.selected||saving===a.id} onClick={()=>select(a.id)}>{saving===a.id?<Loader2 className="animate-spin"/>:null}{a.selected?"Conta selecionada":"Usar esta conta"}</Button></div>)}</div>}
 </div>;
}
