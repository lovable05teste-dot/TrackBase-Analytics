...
export async function PrivateSection({title,description,children}:{title:string;description:string;children?:React.ReactNode}){
 ...
 return <main className="min-h-screen bg-slate-50 text-slate-900 lg:pl-64">
  <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-slate-200 bg-white p-5 lg:block">
   ...
   <div className="absolute bottom-5 left-5 right-5 rounded-xl border border-blue-100 bg-blue-50 p-3"><p className="text-xs font-medium text-blue-700">Dados protegidos</p><p className="mt-1 text-[11px] leading-4 text-slate-500">Eventos e credenciais ficam isolados por projeto.</p></div>
  </aside>
  <div className="min-h-screen"><header className="border-b border-white/[.07] bg-white/90 px-5 py-5 backdrop-blur lg:px-9"><div className="mx-auto flex max-w-[1500px] items-center justify-between"><div><p className="text-xs font-medium uppercase tracking-[.18em] text-blue-600">Workspace</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">{title}</h1><p className="mt-1 text-sm text-slate-500">{description}</p></div><a href="/api/auth/logout" className="rounded-lg border border-slate-200 p-2 text-slate-500 transition hover:border-white/20 hover:text-white" aria-label="Sair"><LogOut className="size-4"/></a></div></header><div className="mx-auto max-w-[1500px] space-y-6 p-5 lg:p-9">{children||<div className="metric-card rounded-2xl p-10 text-center text-slate-400">Nenhum dado recebido ainda.</div>}</div></div>
 </main>;
}