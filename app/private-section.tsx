...
import {LayoutDashboard,PlugZap,BarChart3,ShoppingBag,BookOpen,Settings,LogOut,Volume2} from "lucide-react";
const links=[[LayoutDashboard,"Dashboard","/"],[ShoppingBag,"Vendas","/vendas"],[BarChart3,"Campanhas","/campanhas"],[PlugZap,"Integrações","/integracoes"],[BookOpen,"Documentação","/docs"],[Settings,"Configurações","/configuracoes"]];
export async function PrivateSection({title,description,children}:{title:string;description:string;children?:React.ReactNode}){
 ...
 return <main className="min-h-screen bg-slate-50 text-slate-900 lg:pl-64">
  <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-slate-200 bg-card p-5 lg:block">
   ...
   <nav className="mt-7 space-y-1">{links.map(([Icon,label,href])=>{const I=Icon as typeof LayoutDashboard;return <a key={href as string} href={href as string} className="nav-item"><I className="size-4"/><span>{label as string}</span></a>})}<SoundSidebar/></nav>
