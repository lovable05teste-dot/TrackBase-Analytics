 ...
 return <div className="mx-auto grid max-w-[1100px] gap-6 px-5 py-8 lg:grid-cols-[220px_1fr]">
   ...
   <div className="space-y-6">
    <Sec id="visao" n="Parte 1" title="Visão geral — como a venda chega">
     <p>O GhostScale recebe vendas em 3 peças que trabalham juntas. O gateway só precisa implementar a peça 2; as demais já funcionam sozinhas:</p>
     <Table head={["Peça","O que faz","Quem instala"]} rows={[["1 · Script do navegador","Salva UTMs e cliques por 90 dias, registra PageView, ViewContent e InitiateCheckout, e leva a atribuição até o checkout.","Dono da página de vendas (1 linha de código)"],["2 · Webhook do gateway","Avisa cada venda: aprovada, pendente, reembolsada, chargeback ou cancelada. É a fonte oficial da venda.","Você (dono do gateway) — esta documentação"],["3 · Conversions API","O GhostScale repassa a venda aprovada para a Meta com o mesmo identificador (sem duplicar).","Automático, sem código extra"]]}/>
  ...
  "fbc": "fb.1.123...ABC123",\n  "tb_vid": "visitante-xyz",\n  "utm_source": "facebook",\n  "utm_campaign": "CAMPANHA|123",\n  "event_id": "meu-id-unico-123"\n}`}</Code>
    ...
    <Sec id="campos" n="Parte 4" title="Campos aceitos">
     <p>O GhostScale procura cada informação em vários nomes comuns. Basta enviar UM deles por linha:</p>
    ...
    <Sec id="fortpay" n="Parte 6" title="Caso especial: FortPay">
     ...
     <p>O GhostScale usa <code>transaction.id</code> para atualizar a venda (pendente → aprovada) sem duplicar, converte centavos sozinho e mostra só o <b>nome</b> da campanha (antes do <code>|</code>). Sem UTMs, a campanha fica vazia.</p>
    </Sec>
    <Sec id="valores" n="Parte 7" title="Valores e moeda">
     <p>Se enviar valor em centavos (<code>amount_cents: 6990</code>), o GhostScale divide por 100. Se enviar decimal (<code>amount: 69.90</code>), usa direto. Valores negativos ou inválidos retornam <code>HTTP 400</code>. Moeda padrão: <code>BRL</code>.</p>
    </Sec>
    <Sec id="dedup" n="Parte 8" title="Deduplicação (evitar venda dobrada)">
     <p>Cada venda tem um <code>event_id</code>: se você não enviar, o GhostScale gera <code>purchase_ID-DA-VENDA</code> para aprovadas. O mesmo <code>event_id</code> é usado no navegador (Pixel) e no servidor (CAPI), então a Meta conta uma vez só. Reenvios com o mesmo ID da venda apenas atualizam o registro.</p>
    </Sec>
