...
import {ArrowLeft,Check,Copy,FileText,Megaphone,Radio,PlugZap} from "lucide-react";
...
<Tabs defaultValue="ads" className="w-full"><TabsList className="mb-5 grid h-auto w-full grid-cols-2 rounded-xl border border-white/10 bg-[#101522] p-1 sm:h-14 sm:grid-cols-4"><TabsTrigger value="ads" className="h-11 gap-2 text-sm sm:h-full"><Megaphone className="size-4"/>Anúncios</TabsTrigger><TabsTrigger value="utms" className="h-11 gap-2 text-sm sm:h-full"><FileText className="size-4"/>UTMs</TabsTrigger><TabsTrigger value="pixel" className="h-11 gap-2 text-sm sm:h-full"><Radio className="size-4"/>Pixel & CAPI</TabsTrigger><TabsTrigger value="gateways" className="h-11 gap-2 text-sm sm:h-full"><PlugZap className="size-4"/>Gateways</TabsTrigger></TabsList>
...
<TabsContent value="utms"><Card className="metric-card"><CardHeader><CardTitle>UTMs e script da página</CardTitle></CardHeader><CardContent className="space-y-5"><p className="text-sm leading-6 text-slate-400">Copie e cole no campo <b className="text-slate-200">Parâmetros de URL</b> do anúncio. A Meta preencherá campanha, conjunto, anúncio e posicionamento automaticamente.</p><div className="rounded-xl border border-violet-500/20 bg-black/25 p-4"><code className="block break-all text-sm leading-7 text-sky-300">{metaUtm}</code></div><Button onClick={copyUtm} className="min-w-44">{copied?<><Check/>Copiado</>:<><Copy/>Copiar UTMs</>}</Button><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{[["utm_campaign","Campanha + ID"],["utm_term","Conjunto + ID"],["utm_content","Anúncio + ID"],["placement","Posicionamento"],["site_source_name","Facebook ou Instagram"],["utm_medium","Tráfego pago"]].map(([key,value])=><div key={key} className="rounded-lg border border-white/8 bg-white/[.025] p-3"><code className="text-xs text-violet-300">{key}</code><p className="mt-1 text-sm text-slate-300">{value}</p></div>)}</div><TrackingInstall/></CardContent></Card></TabsContent>
<TabsContent value="pixel"><PixelConnect/></TabsContent><TabsContent value="gateways"><GatewayConnect/></TabsContent>
</Tabs></div></main>}
