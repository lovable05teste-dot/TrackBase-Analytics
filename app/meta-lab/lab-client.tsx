...
import {AlertTriangle,Copy,FlaskConical,History,Loader2,Megaphone,Play,Plus,Trash2,Users} from "lucide-react";
...
type FatigueItem={adName:string;campaignName:string;accountName:string;spend7:number;freqPrev:number;freqRecent:number;ctrPrev:number;ctrRecent:number;verdict:string};
type QualityItem={adName:string;flags?:string[];impressions?:number};
type UtmIssueItem={adName:string;problem:string};
type CompareItem={campaignName:string;metaPurchases:number;metaRevenue:number;tbSales:number;tbRevenue:number;diff:number|null};
type WinnerItem={adName:string;campaignName:string;spend:number;revenue:number;roas:number;accountRowId:string;adId:string};
type DiagnosticsResult={fatigue?:FatigueItem[];quality?:QualityItem[];utmIssues?:UtmIssueItem[];compare?:CompareItem[];winners?:WinnerItem[]};
type AudienceItem={name:string;accountName:string;approximate_count?:number|null};
type HistoryItem={id:string;action:string;actor:string;targetName?:string;targetId?:string;detail?:string;createdAt:number};
...
function Diagnostics(){
 const[res,setRes]=useState<DiagnosticsResult|null>(null);const[loading,setLoading]=useState(false);const[error,setError]=useState("");const[msg,setMsg]=useState("");
...
function Rules(){
 ...
 const[running,setRunning]=useState("");
  const load=async()=>{setLoading(true);setMsg("");try{const r=await fetch("/api/meta/rules",{cache:"no-store"});const b=await r.json();if(!r.ok)throw new Error(b.error||"Falha ao carregar regras.");setRules(b.rules||[])}catch(e){setMsg(e instanceof Error?e.message:"Falha ao carregar regras.")}finally{setLoading(false)}};
 useEffect(()=>{void load();},[]);
 const save=async()=>{setMsg("");try{const r=await apiFetch("/api/meta/rules",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...f,value:Number(f.value),windowDays:Number(f.windowDays),minSpend:Number(f.minSpend),cooldownHours:Number(f.cooldownHours)})});const b=await r.json();if(!r.ok)throw new Error(b.error||"Falha.");setF({...f,name:"",value:"1.5"});setMsg("Regra criada.");await load();}catch(e){setMsg(e instanceof Error?e.message:"Falha.")}};
  const toggle=async(rule:Rule)=>{setMsg("");try{const r=await apiFetch("/api/meta/rules",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({...rule,active:!rule.active})});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error((b as {error?:string}).error||"Falha ao alternar.");await load();}catch(e){setMsg(e instanceof Error?e.message:"Falha ao alternar regra.")}};
  const remove=async(id:string)=>{if(!confirm("Apagar esta regra?"))return;setMsg("");try{const r=await apiFetch(`/api/meta/rules?id=${encodeURIComponent(id)}`,{method:"DELETE"});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error((b as {error?:string}).error||"Falha ao apagar.");await load();}catch(e){setMsg(e instanceof Error?e.message:"Falha ao apagar regra.")}};
...
function Audiences({accounts}:{accounts:Account[]}){
 const[projects,setProjects]=useState<Project[]>([]);const[list,setList]=useState<AudienceItem[]>([]);const[msg,setMsg]=useState("");
 const[accountId,setAccountId]=useState("");const[projectId,setProjectId]=useState("");const[name,setName]=useState("Compradores 180d");const[days,setDays]=useState("180");const[busy,setBusy]=useState(false);
  const loadAll=async()=>{setMsg("");try{const[p,a]=await Promise.all([fetch("/api/projects",{cache:"no-store"}).then(r=>r.json()),fetch("/api/meta/audiences",{cache:"no-store"}).then(r=>r.json()).catch(()=>({audiences:[]}))]);setProjects(p.projects||[]);setList(a.audiences||[]);if(!projectId&&p.projects?.[0])setProjectId(p.projects[0].id);}catch{setMsg("Falha ao carregar projetos/públicos.")}};
 useEffect(()=>{void loadAll();},[]);
 const create=async()=>{const proj=projects.find(p=>p.id===projectId);if(!proj?.pixelId){setMsg("O projeto precisa de um Pixel conectado.");return;}setBusy(true);setMsg("");try{const r=await apiFetch("/api/meta/audiences",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({accountId,pixelId:proj.pixelId,name,days:Number(days)})});const b=await r.json();if(!r.ok)throw new Error(b.error||"Falha.");setMsg(`Público "${b.name}" criado na Meta.`);await loadAll();}catch(e){setMsg(e instanceof Error?e.message:"Falha.")}finally{setBusy(false)}};
 return <div className="space-y-5">
...
export function LabClient(){
 ...
 const accounts=(accs.data?.accounts||[]).map(a=>({id:a.id,adAccountId:a.adAccountId||"",name:a.name}));
  return <div className="space-y-5">
   {accs.error?<p role="alert" className="rounded-lg border border-red-400/25 bg-red-400/10 p-3 text-sm text-red-600">{accs.error} Vincule uma conta em Integrações.</p>:null}
