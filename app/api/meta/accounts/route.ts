...
import { hasActivePlan, hasConflictingOrigin, planRequiredResponse, requestUserId } from "@/lib/trackbase-security";
...
export async function DELETE(request:Request){
 ...
 if(!id)return Response.json({error:"Conta não informada."},{status:400});
 const [account]=await db.select({adAccountId:metaAccounts.adAccountId}).from(metaAccounts).where(and(eq(metaAccounts.id,id),eq(metaAccounts.userId,userId))).limit(1);
 if(!account)return Response.json({error:"Conta não encontrada."},{status:404});
 await db.delete(metaLinked).where(and(eq(metaLinked.userId,userId),eq(metaLinked.adAccountId,account.adAccountId)));
 await db.delete(metaAccounts).where(and(eq(metaAccounts.id,id),eq(metaAccounts.userId,userId)));
 return Response.json({deleted:true,id});
 }catch{return Response.json({error:"Não foi possível desconectar. Tente novamente."},{status:503})}
}
