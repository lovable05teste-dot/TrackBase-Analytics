...
export async function DELETE(request:Request){
 ...
 const accounts=await db.select({id:metaAccounts.id,adAccountId:metaAccounts.adAccountId}).from(metaAccounts).where(and(eq(metaAccounts.userId,userId),eq(metaAccounts.metaUserId,loginId)));
 if(!accounts.length)return Response.json({error:"Login Meta não encontrado."},{status:404});
 const adAccountIds=accounts.map(account=>account.adAccountId);
 await db.delete(metaLinked).where(and(eq(metaLinked.userId,userId),inArray(metaLinked.adAccountId,adAccountIds)));
 await db.delete(metaAccounts).where(and(eq(metaAccounts.userId,userId),eq(metaAccounts.metaUserId,loginId)));
 return Response.json({deleted:true,loginId,accountsRemoved:accounts.length});
