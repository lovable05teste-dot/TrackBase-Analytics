...
export async function POST(request: Request) {
  ...
  await db.insert(events).values({id:crypto.randomUUID(),projectId:credential.projectId,eventId,eventName,source:"gateway",occurredAt:now,value,currency,payload:JSON.stringify(body),visitorId:String(pick(body,["tb_vid","tracking.tb_vid","metadata.tb_vid","data.tracking.tb_vid"])||""),fbclid,fbc,fbp,utmSource:String(pick(body,["utm_source","tracking.utm_source","metadata.utm_source","data.tracking.utm_source"])||""),utmCampaign:String(pick(body,["utm_campaign","tracking.utm_campaign","metadata.utm_campaign","data.tracking.utm_campaign"])||""),utmMedium:String(pick(body,["utm_medium","tracking.utm_medium","metadata.utm_medium","data.tracking.utm_medium"])||""),utmContent:String(pick(body,["utm_content","tracking.utm_content","metadata.utm_content","data.tracking.utm_content"])||""),utmTerm:String(pick(body,["utm_term","tracking.utm_term","metadata.utm_term","data.tracking.utm_term"])||"")}).onConflictDoNothing();
  if(status==="approved"){
    const[project]=await db.select().from(projects).where(eq(projects.id,credential.projectId)).limit(1);
    if(project?.pixelId&&project.metaTokenCipher&&project.metaTokenIv){try{const accessToken=await decryptSecret(project.metaTokenCipher,project.metaTokenIv),email=pick(body,["email","customer.email","data.customer.email","buyer.email"]),phone=pick(body,["phone","customer.phone","data.customer.phone","buyer.phone"]),sourceUrl=String(pick(body,["url","checkout_url","tracking.url","metadata.url"])||"");const capi={data:[{event_name:"Purchase",event_time:now,event_id:eventId,action_source:"website",event_source_url:sourceUrl||undefined,user_data:{client_ip_address:request.headers.get("x-forwarded-for")?.split(",")[0].trim()||undefined,client_user_agent:request.headers.get("user-agent")||undefined,em:await hash(email),ph:await hash(phone,true),fbc:fbc||undefined,fbp:fbp||undefined},custom_data:{value,currency,order_id:externalId}}]};await fetch(`https://graph.facebook.com/v25.0/${project.pixelId}/events?access_token=${encodeURIComponent(accessToken)}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(capi)})}catch(error){console.error("Gateway CAPI",error)}}
  }
