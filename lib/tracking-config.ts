...
export function parseTrackingConfig(raw:unknown):TrackingConfig{
  ...
  return{lead:rule(source.lead,defaultTrackingConfig.lead),addToCart:rule(source.addToCart,defaultTrackingConfig.addToCart),initiateCheckout:rule(source.initiateCheckout,defaultTrackingConfig.initiateCheckout),purchase:{mode:"gateway_approved",valueSource:purchase.valueSource==="fixed"?"fixed":"sale",fixedValue:Math.max(0,Number(purchase.fixedValue)||0),product:text(purchase.product,"any"),thankYouMatch:text(purchase.thankYouMatch,"/obrigado")},gateway:{forwardUtms:bool(gateway.forwardUtms,true),forwardFacebook:bool(gateway.forwardFacebook,true),forwardVisitor:bool(gateway.forwardVisitor,true)},ipMode:source.ipMode==="disabled"||source.ipMode==="ipv4"?source.ipMode:"auto"};
}
